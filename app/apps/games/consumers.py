...
class PongTournoiConsumer(AsyncWebsocketConsumer):
	...
	async def receive(self, text_data):
		...
		if message == 'end':
			...
			player1 = await sync_to_async(CustomUser.objects.get)(username=data['player1'])
			player2 = await sync_to_async(CustomUser.objects.get)(username=data['player2'])
			if player1 != winner:
			...
			if await sync_to_async(lambda: tournament.match_entries.count())() == 0:
				...
				self.send_to_all('end tournament the winner is' + winner.username)
				#delete le tournoi
				await sync_to_async(tournament.delete)()
				return
			self.send_to_all('next_match')
			return
		await self.send_to_all(message, player, ball, playery, ballx, bally, score1, score2)
